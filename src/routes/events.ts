...
events.post("/listen/start", async (context) => {
  ...
  const { visitor_id: visitorId, visit_id: visitId, channel_id: channelId } = parsed.data;
  ...
  await db
    ...
    .bind(sessionId, visitorId, visitId, channelId, now, now)
    .run();
...
events.post("/listen/heartbeat", async (context) => {
  ...
  return applyListenProgress(context, parsed.data.session_id, { markEnded: false });
});
...
events.post("/listen/end", async (context) => {
  ...
  return applyListenProgress(context, parsed.data.session_id, { markEnded: true });
});
...
async function applyListenProgress(
  ...
  const session = await db
    .prepare("SELECT visitor_id, channel_id, last_heartbeat_at, ended_at FROM listen_sessions WHERE id = ?")
    .bind(sessionId)
  ...
  const updateSql = options.markEnded
    ? "UPDATE listen_sessions SET ended_at = ?, last_heartbeat_at = ?, duration_seconds = duration_seconds + ? WHERE id = ?"
    : "UPDATE listen_sessions SET last_heartbeat_at = ?, duration_seconds = duration_seconds + ? WHERE id = ?";
  const updateArgs = options.markEnded
    ? [now, now, elapsedSeconds, sessionId]
    : [now, elapsedSeconds, sessionId];

  await db.batch([
    db.prepare(updateSql).bind(...updateArgs),
    db
      .prepare(
        `INSERT INTO visitor_daily_listen (visitor_id, listen_date, channel_id, seconds)
         VALUES (?, ?, ?, ?)
         ON CONFLICT(visitor_id, listen_date, channel_id) DO UPDATE SET seconds = seconds + excluded.seconds`,
      )
      .bind(session.visitor_id, listenDate, session.channel_id, elapsedSeconds),
